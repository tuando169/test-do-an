...
export default function Pricing() {
  ...
  const handleButtonClick = async (item) => {
    ...
    try {
      const returnUrl = `${window.location.origin}/manage-space`;
      const cancelUrl = window.location.href;

      const data = await PaymentApi.createPaymentLink(item.id, returnUrl, cancelUrl);
      ...
      api.error({ message: "Lỗi tạo giao dịch", description: "Vui lòng thử lại sau." });
      setProcessingId(null);
    }
  };

  return (
    <div className="container-main flex justify-center py-16 bg-white w-full px-4">
      {contextHolder}
        ...
        <div className="w-full flex flex-nowrap gap-4 justify-center">
          {displayPlans.map((item, i) => {
            ...
            return (
              <div
                key={i}
                style={{ flexBasis: `${100 / displayPlans.length}%` }}
                className={`
              ...
              >
                ...
                <button 
                  ...
                  disabled={(!!processingId && processingId === item.id) || isActive}
                  className={`
                ...
                >
                  {isActive 
                    ? "✓ ĐANG SỬ DỤNG" 
                    : (processingId === item.id ? "ĐANG XỬ LÝ..." : item.button)
                  }
                </button>
              </div>
            );
